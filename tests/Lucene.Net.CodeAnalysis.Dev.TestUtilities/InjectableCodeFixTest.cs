...
{
    public class InjectableCodeFixTest : CodeFixTest<Verifier>
    {
        private readonly Func<DiagnosticAnalyzer> analyzerFactory;
        private readonly Func<CodeFixProvider> codeFixFactory;

        public InjectableCodeFixTest(Func<DiagnosticAnalyzer> analyzerFactory, Func<CodeFixProvider> codeFixFactory)
        {
            this.analyzerFactory = analyzerFactory ?? throw new ArgumentNullException(nameof(analyzerFactory));
            this.codeFixFactory = codeFixFactory ?? throw new ArgumentNullException(nameof(codeFixFactory));
        }
